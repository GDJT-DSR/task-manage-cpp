...
std::string answer::validate(const orm::Row &question,
    ...
    if (!has_permission(state, submit | view_page)) {
        return "权限不足";
    }
...
Task<> answer::text(HttpRequestPtr req,
    ...
    try {
        ...
        LOG_ERROR << "error occurred when submitting answer";
    }
    callback(response::fail(k500InternalServerError, "服务器错误"));
    co_return;
}

Task<> answer::upload(HttpRequestPtr req,
                      std::function<void(const HttpResponsePtr &)> callback,
                      int id) {
    ...
    try {
        ...
        const auto name =
            utils::getUuid() + '.' + std::string(file.getFileExtension());
        if (file.saveAs(name) != 0) {
            callback(response::fail(k500InternalServerError, "保存失败"));
        }
        const auto answers =
        ...
        if (answers.empty()) {
            ...
            if (!std::filesystem::remove(uploadPath / originFile)) {
                LOG_ERROR << std::format("error occurred when removing upload file : {}", originFile);
            }
        ...
        // client->execSqlCoro()
        // auto a = co_await client->execSqlCoro("abc", "ncd", "lsl");
        ...
        LOG_ERROR << "error occurred when uploading file";
    }
