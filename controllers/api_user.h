...
namespace api {
  ...
  public:
    ...
    static Task<> login(HttpRequestPtr req, std::function<void(const HttpResponsePtr &)> callback,
                        std::optional<dto::user> user);

    static Task<> changePassword(HttpRequestPtr req, std::function<void(const HttpResponsePtr &)> callback,
                                 std::optional<dto::user_change_pwd> data);

    static Task<> refreshToken(HttpRequestPtr req, std::function<void(const HttpResponsePtr &)> callback);
  };
}
...
namespace drogon {
  ...
  inline std::optional<dto::user_change_pwd> fromRequest(const HttpRequest &req) {
    ...
    if (const Json::Value &origin = (*json)["origin"], &target = (*json)["target"];
      ...
      return {{origin.asString(), target.asString()}};
    }
    return {};
  }
} // namespace drogon
