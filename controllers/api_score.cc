...
Task<> score::getAll(HttpRequestPtr req,
    ...
    if (!permission::has_permission(permission,
                                    permission::user_permission::score)) {
    ...
    try {
        auto questions = co_await client->execSqlCoro(
            ...
            "JOIN scorer_question ON questions.id = "
            "scorer_question.question_id "
            "WHERE scorer_question.user_id = $1",
            uid);
        if (questions.empty()) {
            co_return callback(
                response::success(Json::Value(Json::arrayValue)));
        }
        std::unordered_map<int, Json::Value> map;
        for (const auto &question : questions) {
            const int pid = question["page_id"].as<int>();
            if (!map.contains(pid)) {
                map.insert({pid, Json::Value(Json::arrayValue)});
            }
            Json::Value val;
        ...
        // 查询page
        const auto pages =
            co_await client->execSqlCoro<std::string>(clause, pids);
        Json::Value resp(Json::arrayValue);
        for (const auto &page : pages) {
            const int64_t state = page["state"].as<int64_t>();
            if (!permission::has_permission(
                    state, permission::enable | permission::reviewer_visible)) {
                continue;
            }
            Json::Value single;
            ...
            single["reviewable"] =
                has_permission(state, permission::reviewable);
...
Task<> score::getSingle(HttpRequestPtr req,
                        std::function<void(const HttpResponsePtr &)> callback,
                        int id) {
    ...
    try {
        ...
        if (!has_permission(state, permission::score)) {
            co_return callback(response::fail(k403Forbidden, "权限不足"));
        }
        Json::Value resp;
        resp["type"] = type;
...
Task<>
score::submitScore(HttpRequestPtr req,
                   const std::function<void(const HttpResponsePtr &)> callback,
                   const int id, const std::optional<dto::Score> score) {
    if (!score) {
        co_return callback(response::fail(k400BadRequest, "参数错误"));
    }
    const auto &uid = req->getParameter("id");
    const auto permission =
        transform::bstring2int<int64_t>(req->getParameter("permission"));
    ...
    try {
        const auto list = co_await client->execSqlCoro(
            ...
            "WHERE answers.id = $1 AND scorer_id = $2",
            id, uid);
        if (list.empty()) {
            co_return callback(response::fail(k403Forbidden, "无权访问"));
        }
        const auto info = list.front();
        // 验证权限
        const auto value = score->target;
        if (!validateScore(value, info)) {
            co_return callback(response::fail(k400BadRequest, "参数不合法"));
        }
        const auto resp = co_await client->execSqlCoro(
            "UPDATE answers SET score = $1 WHERE id = $2 AND score = $3", value,
            id, score->origin);
        ...
        co_return callback(
            response::fail(k409Conflict, "数据发生变化，请重新尝试"));
    } catch (std::exception &e) {
        LOG_ERROR << e.what();
...
bool score::validateScore(int val, const orm::Row &row) {
    ...
    if (!has_permission(state, enable | reviewable)) {
        return false;
    }
    if (val > max_score) {
        return false;
