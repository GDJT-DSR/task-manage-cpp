    ...
    try
    {
        ...
        for (const auto& page : pages)
        {
            ...
            if (!permission::has_permission(
                state, permission::enable | permission::reviewer_visible)) { continue; }
            Json::Value single;
                   ...
                   const int id, const std::optional<dto::Score> score)
{
    ...
    try
    {
        const auto list = co_await client->execSqlCoro(
            "SELECT max_score,score_step,state,score FROM answers "
        ...
        const auto sc = info["score"];
        if (sc.isNull() || sc.as<int>() == score->origin)
        {
            // 验证权限
            ...
            const auto resp = co_await client->execSqlCoro(
                "UPDATE answers SET score = $1 WHERE id = $2", value,
                id);
