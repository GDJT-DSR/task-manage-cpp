...
Task<> setToken(const std::function<void(const HttpResponsePtr &)> &callback,
    ...
    co_await redis->execCommandCoro(
        "set auth:user_refresh_token:%lld %s EX %lld", id, refreshToken.c_str(),
        expires);
...
Task<> user::login(HttpRequestPtr req,
    ...
    try {
        ...
        const auto &result = co_await client->execSqlCoro(
            "SELECT id,username,permission,password,updated_at FROM users "
            "WHERE "
            "username = $1 LIMIT 1",
            user->username);
        if (result.empty()) {
        ...
        if (!encoder->verify(user->password,
...
Task<> user::refreshToken(
    ...
    try {
        ...
        // id.value()});
        const auto &result = co_await client->execSqlCoro(
            "SELECT id,updated_at,permission FROM users WHERE id = $1", id);
        if (result.size() != 1) {
...
Task<> user::logout(HttpRequestPtr req,
                    std::function<void(const HttpResponsePtr &)> callback) {
    ...
    resp->addCookie(cookie);
    // 从redis中移除，防止该refresh token继续被使用
    const auto id = req->getParameter("id");
    const auto redisClient = app().getFastRedisClient();
    if (redisClient) [[likely]] {
        // redisClient->execCommandAsync([](const nosql::RedisResult&res,
        // std::string_view command, ...)
        redisClient->execCommandAsync(
            [](const nosql::RedisResult &res) {},
            [](const nosql::RedisException &e) {
                LOG_ERROR << "logout error:" << e.what();
            },
            "DEL auth:user_refresh_token:%s", id.c_str());
    } else {
        LOG_ERROR << "redis is null";
    }
